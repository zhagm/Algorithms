...
that indicates the character that is to be counted (rather than counting only uppercase “B” characters). Rewrite countBs to make use of this new function.
*/

function countBs(str) {
  let bCount = 0;
  for (let letter of str) bCount += letter === "B";
  return bCount;
}

function countChar(str, char) {
  let count = 0;
  for (let letter of str) count += letter === char;
  return count;
}
